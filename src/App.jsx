...
function BackupRestoreModal({ onClose, updateStaff, updateEntries, updateDailyStats, updatePtoBalances, updateNonWorkTypes, updateAlertSettings, updateDayNotes, updateVisitData }) {
  ...
  const restore = async (file) => {
    ...
    try {
      ...
      const newVisits = {};
      visitRows2.filter(r=>r[0]&&r[1]&&r[2]).forEach(r => {
        const [key, weekStart, team, evals, visits] = r;
      ...
      if (newNW.length) updateNonWorkTypes(newNW);
      if (Object.keys(newAlerts.fteTargets).length) updateAlertSettings(newAlerts);
      updateDayNotes(newNotes);
      if (Object.keys(newVisits).length) updateVisitData(newVisits);
      const visitWeekCount = Object.keys(newVisits).length;
      setStatus({ok:true, msg:`✅ Restored! ${newStaff.length} staff · ${Object.keys(newEntries).length} schedule entries · ${Object.keys(newDailyStats).length} daily stats${visitWeekCount ? ` · ${visitWeekCount} visit weeks` : ""}.`});
    } catch(err) {
